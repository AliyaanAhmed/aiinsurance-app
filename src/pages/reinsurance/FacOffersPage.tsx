...
interface FacOfferRecord {
  ...
  statusReason: 'Accepted' | 'Declined'
}

interface FacOfferCreateFormState {
  name: string
  isLeader: string
...
const emptyFacOfferCreateForm: FacOfferCreateFormState = {
  name: '',
  isLeader: '',
...
const placementOptions = [
  { value: 'Select', label: 'Select placement' },
  { value: 'PL-001', label: 'PL-001' },
]

const reinsurerOptions = [
  { value: 'Select', label: 'Select reinsurer' },
  { value: 'Delta Reinsurance', label: 'Delta Reinsurance' },
...
const facOffers: FacOfferRecord[] = [
  {
    id: 'fac-offer-fo-001',
    name: 'FO-001',
    isLeader: 'Yes',
    offeredSharePercentage: 0.4,
    placementId: 'PL-001',
    quotedRate: 0.01,
    reinsurerId: 'Delta Reinsurance',
    responseDate: '2026-08-08',
    signedLinePercentage: 0.35,
    termsAndConditions: '---',
    writtenLinePercentage: '---',
  ...
  },
  {
    ...
    name: 'FO-002',
    isLeader: 'No',
    offeredSharePercentage: 0.3,
    placementId: 'PL-001',
    quotedRate: 0.01,
    reinsurerId: 'Delta Reinsurance',
    responseDate: '2026-08-09',
    signedLinePercentage: 0.26,
    termsAndConditions: '---',
    writtenLinePercentage: '---',
    statusReason: 'Accepted',
  },
  {
    ...
    name: 'FO-003',
    isLeader: 'No',
    offeredSharePercentage: 0.25,
    placementId: 'PL-001',
    quotedRate: 0.01,
    reinsurerId: 'Meridian Re',
    responseDate: '2026-08-10',
    signedLinePercentage: 0.17,
    termsAndConditions: '---',
    writtenLinePercentage: '---',
    statusReason: 'Accepted',
  },
  ...
  },
  {
    ...
    name: 'FO-005',
    isLeader: 'No',
    offeredSharePercentage: 0.2,
    placementId: 'PL-001',
    quotedRate: 0,
    reinsurerId: 'Meridian Re',
    responseDate: '2026-08-09',
    signedLinePercentage: 0,
    termsAndConditions: '---',
    writtenLinePercentage: '---',
...
const tableColumns: Array<{
  ...
  { key: 'writtenLinePercentage', label: 'Written Line Percent...', render: (record) => record.writtenLinePercentage },
  { key: 'statusReason', label: 'Status Rea...' },
]

export function FacOffersPage() {
  const [records, setRecords] = useState(facOffers)
    ...
    <div className="space-y-6">
                ...
                >
                  {tableColumns.map((column) => (
                    <td key={column.key} className="px-4 py-4 align-middle text-[13px]">
                      ...
                      ) : column.key === 'placementId' || column.key === 'reinsurerId' ? (
                        <span className="font-semibold text-primary">
                          {column.render ? column.render(record) : String(record[column.key])}
                        </span>
                      ) : column.key === 'statusReason' ? (
                        <Badge variant={record.statusReason === 'Accepted' ? 'approved' : 'rejected'}>
                          {record.statusReason}
                        </Badge>
                      ) : (
      ...
      {isCreateOpen ? (
        <CreateFacOfferModal
          ...
          onCreate={(form) => {
            const created: FacOfferRecord = {
              id: `fac-offer-${Date.now()}`,
              ...
              placementId: resolveSelectValue(form.placementId, '---'),
              quotedRate: parseNumberInput(form.quotedRate),
              reinsurerId: resolveSelectValue(form.reinsurerId, '---'),
              responseDate: form.responseDate || new Date().toISOString(),
              signedLinePercentage: parseNumberInput(form.signedLinePercentage),
              termsAndConditions: form.termsAndConditions.trim() || '---',
              writtenLinePercentage: form.writtenLinePercentage.trim() || '---',
              statusReason: resolveStatusReason(form.statusReason, 'Accepted'),
            }
...
function CreateFacOfferModal({
  ...
  onClose: () => void
  onCreate: (form: FacOfferCreateFormState) => void
}) {
  const [form, setForm] = useState<FacOfferCreateFormState>(emptyFacOfferCreateForm)

  const updateForm = <Key extends keyof FacOfferCreateFormState>(
    key: Key,
    value: FacOfferCreateFormState[Key],
  ) => {
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-border-soft px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Add offer share, placement, reinsurer response, signed line, and status details.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="scrollbar-sleek flex-1 overflow-y-auto px-5 py-4">
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <ModalField label="Offered Share Percentage">
              <Input
                value={form.offeredSharePercentage}
                onChange={(event) => updateForm('offeredSharePercentage', event.target.value)}
                placeholder="Enter offered share percentage"
              />
            </ModalField>
            <ModalField label="Placement Id">
              <Select
                value={form.placementId}
                onValueChange={(value) => updateForm('placementId', value)}
                options={placementOptions}
                placeholder="Select"
              />
            </ModalField>
            <ModalField label="Quoted Rate">
              <Input
                value={form.quotedRate}
                onChange={(event) => updateForm('quotedRate', event.target.value)}
            ...
            <ModalField label="Response Date">
              <CalendarDateField
                value={form.responseDate}
                onChange={(value) => updateForm('responseDate', value)}
              />
            </ModalField>
            <ModalField label="Signed Line Percentage">
              <Input
                value={form.signedLinePercentage}
                onChange={(event) => updateForm('signedLinePercentage', event.target.value)}
                placeholder="Enter signed line percentage"
              />
            </ModalField>
            <ModalField label="Terms and Conditions">
              <Input
            ...
            <ModalField label="Written Line Percentage">
              <Input
                value={form.writtenLinePercentage}
                onChange={(event) => updateForm('writtenLinePercentage', event.target.value)}
                placeholder="Enter written line percentage"
              />
            </ModalField>
            <ModalField label="Status Reason">
              <Select
                value={form.statusReason}
                onValueChange={(value) => updateForm('statusReason', value)}
    ...
    <div className="relative">
      ...
      >
        <span className={value ? '' : 'text-[#94A3B8]'}>
          {selectedDate ? formatDate(selectedDate.toISOString()) : 'Select response date'}
        </span>
...
function FacOfferDetail({
  ...
  onBack: () => void
  onSave: (record: FacOfferRecord) => void
}) {
  const [isEditing, setIsEditing] = useState(false)
  const [form, setForm] = useState<FacOfferCreateFormState>(() => toFacOfferForm(record))

  const updateForm = <Key extends keyof FacOfferCreateFormState>(key: Key, value: FacOfferCreateFormState[Key]) => {
    setForm((current) => ({ ...current, [key]: value }))
  }
  ...
  const saveEdit = () => {
    onSave({
      ...
      offeredSharePercentage: parseNumberInput(form.offeredSharePercentage),
      placementId: resolveSelectValue(form.placementId, record.placementId),
      quotedRate: parseNumberInput(form.quotedRate),
      reinsurerId: resolveSelectValue(form.reinsurerId, record.reinsurerId),
      responseDate: form.responseDate || record.responseDate,
      signedLinePercentage: parseNumberInput(form.signedLinePercentage),
      termsAndConditions: form.termsAndConditions.trim() || '---',
      writtenLinePercentage: form.writtenLinePercentage.trim() || '---',
      statusReason: resolveStatusReason(form.statusReason, record.statusReason),
    })
    setIsEditing(false)
  }
        ...
        actions={
          isEditing ? (
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Button onClick={saveEdit}>Save Fac Offer</Button>
            </div>
          ) : (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {isEditing ? (
            <>
              ...
              <EditableField label="Placement Id"><Select value={form.placementId} onValueChange={(value) => updateForm('placementId', value)} options={placementOptions} /></EditableField>
              ...
              <EditableField label="Reinsurer Id"><Select value={form.reinsurerId} onValueChange={(value) => updateForm('reinsurerId', value)} options={reinsurerOptions} /></EditableField>
            ...
            <>
              ...
              <ReadOnlyField label="Placement Id" value={record.placementId} />
              ...
              <ReadOnlyField label="Written Line Percentage" value={record.writtenLinePercentage} />
...
function ReadOnlyField({
  ...
  return (
    <div className="min-w-0">
      ...
      <div className={`form-field-surface rounded-xl border border-border-soft px-4 py-2.5 text-sm text-foreground ${multiline ? 'min-h-[82px]' : 'min-h-[42px]'}`}>
        {value}
      </div>
    </div>
  )
...
function toFacOfferForm(record: FacOfferRecord): FacOfferCreateFormState {
  return {
    name: record.name,
...
function resolveStatusReason(value: string, fallback: 'Accepted' | 'Declined'): 'Accepted' | 'Declined' {
  return value === 'Accepted' || value === 'Declined' ? value : fallback
}
